...
- **step 1** — 暗场远景粒子云 + 钩子字幕"我刷到一张图，愣了三秒"
...
- **step 2** — 排行榜慢镜景深聚焦 + 主分数 hero 数字 blur clear 浮出
  ...
  · 素材补：主分数（来自 material §1，具体数字）+ 测评窗口（"X 月 N 日 ~ Y 日"）+ 投票数（mono cue 角标）—— 讲者只说"换榜首"，画面把"领先多少 / 多少票投出来的"全挂上

- **step 3** — 第 2 名对比横条 + "+差距分"慢浮锐化
  ...
  · 素材补：第 2 名具体名字（material §1）+ 差距分（具体数字 vs 模糊"低很多"）+ 趋势注释（"过去 N 周首次反超"）

- **step 4** — 官方原话 pull-quote 慢镜入场（电影感引文）
  ...
  · 素材补：原话直引（来自 material §1，1~2 句）+ 落款来源（"— 出处.AI"）—— 引文是素材里讲解完全省略的"权威背书"

- **step 5** — 主持人介绍 + 4 件事预告速览
...
- **step 1** — hero"强在哪 · 四个方向" + 4 个 ghost 占位卡
  ...
  · 素材补：4 个方向各自的关键词（mono cue 标签，"01 X / 02 Y / 03 Z / 04 W"）

- **step 2** — 第 1/4 项填实 + 大图慢镜 takeover
  ...
  · 素材补：本项的具体表现（material §2 抽 1~2 个数据点 / 案例标签）—— 讲者只说"它强在 X"，画面挂"具体强到 N% / 跑赢 M / 测评分数 K"

- **step 3** — 第 2/4 项填实 + 列表/演示
  ...
  · 素材补：4 行细则的具体内容（来自 material §2 第 N 段子列表）—— 讲者只说"指令遵循好"，画面把素材列出来的 4 个具体维度"主体放哪 / 背景怎么搭 / ..."逐行打出来

- **step 4** — 第 3/4 项填实 + before/after 慢镜对照 + 永动 cross-fade
- **step 5** — 第 4/4 项填实 + 多参数预览 + redacted 注释
...
> **观察**：每个 step 的画面都做到"讲者说一件事，画面挂多件事"。比如
> step 2 讲者只是"第一项很强"，画面同时呈现：本项关键词 / 大图实例 /
