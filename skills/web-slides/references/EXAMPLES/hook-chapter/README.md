...
> 开工准备。本 anchor 给的是"钩子型开场的结构骨架"——你要保留它的
...
开场最常用的章节类型：**抛 N 张可疑图 / 反例 / 截图 → 引出主题 →
切大字 hero takeover**。
...
- **step 1** — N 张可疑图片占位（虚线 ghost 卡片）
- **step 2** — 第 1 张露出：<反例 1 描述>（独占视觉）
- **step 3** — 第 2 张露出：<反例 2 描述>（独占视觉）
- **step 4** — 第 3 张露出：<反例 3 描述>（独占视觉）
- **step 5** — 三张图同时缩入侧栏，中间出 <主题大字> takeover
- **step 6** — 切到下一句钩子（被 brush 划掉）
```
...
讲者会**一个一个分开讲** —— 必须 1 项 = 1 step（[CHAPTER-CRAFT.md「逐步揭示」](../../CHAPTER-CRAFT.md#逐步揭示禁止一次全展示)）。
同时 stagger 上 = 观众扫一眼看完，讲者还在讲第一张 = PPT 直觉。
