...
> **正确使用流程**：
>
> 1. 走完 [`../CHAPTER-CRAFT.md`](../CHAPTER-CRAFT.md) 开工准备
...
> 「避免 AI 味」里「整章只用一种入场动画」同质化反模式（每套 slides
> 看起来像同一个模板的 N 个变奏）。
...
| [`list-reveal/`](list-reveal/) | **列举型** —— 讲"三件事 / N 个特性"，每项 1 step | `chapter.tsx` + `chapter.css` |

每个 example 都是**完整章节**：**内容驱动主导动作** + 必要的伴随动作
（**不强求挂持续微动**，按 [`../CHAPTER-CRAFT.md`](../CHAPTER-CRAFT.md)
「放映基本审美」中的内容驱动动画 节制使用）、真素材（不是占位卡）、字号狠对比、绑了
`newsroom` 主题作为示范。
...
| 例子 | 题材 | 文件 |
|---|---|---|
| [`case-tech-review/`](case-tech-review/) | 科技测评 / 实测对比 / 跑分类内容 | README + outline 节选 |
...
「CSS / SVG / Canvas / JS 视觉演示」一节（CSS / SVG / Canvas / JS 全栈）里覆盖了；按 anchor
的"形"组合即可。
