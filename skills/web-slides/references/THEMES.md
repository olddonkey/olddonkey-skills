...
每套 slides 从头到尾跑**一个主题**。我们**不**在章节间翻转明暗 —— 那会
打断放映的视觉连贯性，切到下一章时看起来像很硬的剪辑。如果想要"暗一点
的氛围"段落，请在**同一调色板内**降对比、收聚光，而不是翻转表面色。
...
| `clay-editorial`     | vintage-editorial 的克制孪生。暖象牙底 + 陶土色（#CC785C）accent。Fraunces 正体（不斜）+ Inter 正文。**无装饰 + 140×100 大留白**，hero 数字 weight 400 极克制。像现代研究机构官网。慢速平滑节奏。                |
...
```bash
cp <path-to-web-slides>/themes/newsroom/tokens.css \
...
`stage-frame::after`，vignette 用 `stage-frame::before`），所以放映 /
截图 / 投屏时都看得到。
