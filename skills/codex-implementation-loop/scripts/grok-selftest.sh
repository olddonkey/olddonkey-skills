#!/usr/bin/env bash
# Regression harness for grok-dispatch.sh and grok-verify-worktree.sh.

set -euo pipefail

SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P)"
DISPATCH="$SCRIPT_DIR/grok-dispatch.sh"
VERIFIER="$SCRIPT_DIR/grok-verify-worktree.sh"
...
mkdir -p "$ADAPTER_DIR"
cp "$DISPATCH" "$ADAPTER_DIR/grok-dispatch.sh"
cp "$VERIFIER" "$ADAPTER_DIR/grok-verify-worktree.sh"
python3 - "$ADAPTER_DIR/grok-dispatch.sh" <<'PY'
...
TEST_DISPATCH="$ADAPTER_DIR/grok-dispatch.sh"
...
expect_contains "--background is companion-only" "help documents foreground-only operation"
...
expect_contains "companion-only" "--background refusal explains lifecycle owner"
