...
shipping implementations are `codex-dispatch.sh` and `grok-dispatch.sh`.
...
Per-backend flags beyond the contract are allowed where a real capability exists and must not contradict the contract. Environment-variable overrides are namespaced per backend (`CODEX_LOOP_*` and `GROK_LOOP_*`).
