...
- All parsing reads an ANSI-stripped view of the log, so forced-color output (`pytest --color=yes`) is recognized normally; the log file on disk keeps the raw bytes.
