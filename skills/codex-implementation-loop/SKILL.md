...
- **Service tier is a third speed lever, separate from model and effort.** Codex supports priority routing (**Fast**, request value `priority`) and cheaper-but-slower `flex`, set as `service_tier` in `~/.codex/config.toml` or via the codex TUI's model picker; support varies by model. Same model, same reasoning — different queue: model trades capability, effort trades thinking depth, tier trades routing speed/cost. The companion has no per-task tier flag, so every dispatch inherits whatever the user's config says — if they want Codex runs faster or cheaper, that's the knob to point them at. Ask once at calibration; a mid-loop tier change is a conditions change worth noting, not something to do silently.
