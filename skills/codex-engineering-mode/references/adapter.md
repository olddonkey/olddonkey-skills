...
| --- | --- | --- |
| `investigation dispatch` | Select the kernel's `read-only` dispatch mode with `--read-only` on the selected backend's dispatch script — `codex-dispatch.sh` (default), `grok-dispatch.sh`, or `cursor-dispatch.sh` (all under `skills/codex-implementation-loop/scripts/`). | `skills/codex-implementation-loop/SKILL.md` §Dials, “Backend” and “Dispatch mode,” and §2, “Dispatch” |
